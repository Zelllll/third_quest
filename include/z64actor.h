...
// skin : skin value from DungeonPillarSkins
// initDir : sets if the pillar is initially rising or falling (first rise = 0, first fall = 1)
// speed : units to move per frame (minus one)
// reverse : if true the pillar will crush instead of rise
// playerCheck : if true, only rise / fall if the player is standing on the pillar
// switch flag : switch flag for activating the pillar
#define DP_PARAMS(skin, initDir, speed, reverse, playerCheck, switchFlag) ((skin << 12) | (initDir << 11) | (speed << 8) | (reverse << 7) | (playerCheck << 6) | (switchFlag))

// scale : scale value between 0 and 255 (minus one)
// waitFrames : frames to wait after stopping movement before moving again
#define DP_X_ROT(scale, waitFrames) ((scale << 8) | (waitFrames))

// height : total distance in units to move
#define DP_Z_ROT(height) (height)
...
// type : type value from TorchTypes
// cutsceneFlag : sets if a cutscene should play when the torch is lit (only applies to permanently lit torches)
// isLitFlag : sets if the torch should always be lit
// puzzleTorchTotal : total amount of torches that need to be lit for a puzzle
// switchFlag : switch flag used for puzzles
#define TORCH_PARAMS(type, cutsceneFlag, isLitFlag, puzzleTorchTotal, switchFlag) ((type << 13) | (cutsceneFlag << 12) | (isLitFlag << 11) | (puzzleTorchTotal << 6) | (switchFlag))

// used for decorative torches that are always lit and not used for puzzles
#define TORCH_LIT_PARAMS(type) TORCH_PARAMS(type, false, true, 0, 0x3F)
